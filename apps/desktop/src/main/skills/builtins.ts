...
export const screenQuestionSkill: MaxSkill = {
  ...
  async execute(_match, context) {
    ...
    const response = observation.activeApplication
      ? `Henrique, a janela ativa é ${observation.activeApplication}${observation.activeWindowTitle ? `, ${observation.activeWindowTitle}` : ''}.`
...
export const youtubeSearchSkill: MaxSkill = {
  ...
  priority: 94,
  match: slot(
    /^(?:(?:pesquise|pesquisar|pesquisa|procure|procurar|busque|buscar)\s+(?:no\s+)?youtube(?:\s+por)?|youtube\s+(?:pesquise|procure|busque)(?:\s+por)?)\s+(.+)$/,
    'query'
  ),
  async execute(match, context) {
    const query = match.slots?.query ?? ''
