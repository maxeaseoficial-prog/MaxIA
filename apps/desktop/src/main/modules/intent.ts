...
export function stripWakeWord(value: string): string {
  return normalizeText(value)
    .replace(/^(ei|hey|e)\s+(max|mais|mex)\b\s*/i, '')
    .trim()
}

export function hasWakeWord(value: string): boolean {
  return /\b(?:ei|hey|e)\s+(?:max|mais|mex)\b/i.test(normalizeText(value))
}
...
export function classifyCommand(raw: string): MaxIntent {
  ...
  if (new RegExp(`^${open}\\s+(o\\s+)?(meu\\s+)?navegador$`).test(text)) return { type: 'open-browser' }
  if (new RegExp(`^${open}\\s+(o\\s+)?google$`).test(text)) return { type: 'open-google' }
  if (new RegExp(`^${open}\\s+(o\\s+)?(seu\\s+)?cerebro$`).test(text)) return { type: 'open-brain' }
  if (/^(o que|oque).*(na|minha) tela|o que.*aparecendo.*tela|olha isso/.test(text)) return { type: 'screen-question' }

  const app = text.match(new RegExp(`^${open}\\s+(?:o\\s+|a\\s+)?(.+)$`))
